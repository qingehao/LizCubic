...
static char link_num_addr[SIM76XX_MAX_CONNECTIONS][25] = {0}; // 存储ip地址和link序号之间的对应关系
...
static void at_tcp_ip_errcode_parse(int result)
{
    ...
    case 3 :
        ...
        break;
    case 4 :
        LOG_D("%d : Operation not supported",          result);
        break;
    case 5 :
...
#define CLOSE_COUNTS   5
    ...
    rt_memset(link_num_addr[device_socket], 0, 30);
__exit:
    if (resp)
    {
        at_delete_resp(resp);
...
MSH_CMD_EXPORT(list_link_ip, list);
/**
...
__retry:
    ...
    rt_snprintf(link_num_addr[device_socket], 28, "%s:%d", ip, port);
...
static int get_linkInd_by_ip(const char *uri)
{
    for (int i = 0; i < 10; i++)
    {
        if (rt_strstr(uri, link_num_addr[i]) != RT_NULL)
        {
            return i;
        }
    }
...
static void urc_recv_from_func(struct at_client *client, const char *data, rt_size_t size)
{
    char txt[30] = {0};
    int linkInd = 0;
    struct at_device *device = RT_NULL;
    ...
    sim76xx = (struct at_device_sim76xx *) device->user_data;
    rt_memcpy(txt, data + 10, size);
    sim76xx->linkInd = get_linkInd_by_ip(txt);
}
