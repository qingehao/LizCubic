...
static void temp_notify(int arg)
{
...
#define CGATT_RETRY                    10
    ...
    if(sim76xx->notification == RT_NULL)
    {
        sim76xx->notification = temp_notify;
    }
    ...
    while (retry_num--)
    {
        ...
        for (i = 0; i < CPIN_RETRY; i++)
        {
            ...
            rt_thread_mdelay(1000);
            sim76xx->notification(LTE_SIM_FAIL); // 通知SIM卡事件
        }
        ...
        for (i = 0; i < CSQ_RETRY; i++)
        {
            ...
            sim76xx->notification(LTE_CSQ_FAIL); // 通知信号事件
            rt_thread_mdelay(1000);
        }
        ...
        AT_SEND_CMD(client, resp, "AT+CIPSRIP=1");
        ...
        for (i = 0; i < CREG_RETRY; i++)
        {
            ...
            if (!strncmp(parsed_data, "0,1", sizeof(parsed_data)) || !strncmp(parsed_data, "0,5", sizeof(parsed_data))\
                    || !strncmp(parsed_data, "0,6", sizeof(parsed_data)))
            {
                LOG_D("%s device GSM is registered(%s).", device->name, parsed_data);
                break;
            }
            sim76xx->notification(LTE_NET_FAIL); // 通知网络失败
            rt_thread_mdelay(1000);
        }
...
__exit:
    ...
    if (result == RT_EOK)
    {
        ...
        sim76xx->notification(LTE_NET_SUCCESS); // 通知网络成功
    ...
    sim76xx->notification(LTE_INIT_DONE); // 通知网络成功
}

int sim76xx_ntp_sync()
{
#define SIM76XX_NTP_RESP_SIZE   64
#define SIM76XX_NTP_RESP_TIMO   (8 * RT_TICK_PER_SECOND)
#define NTP_RETRY    10
    at_response_t resp = RT_NULL;
    int result_code = -1;
    int ret = 0;
    ...
    device = at_device_get_by_name(AT_DEVICE_NAMETYPE_NETDEV, "sim1");
    if (device == RT_NULL)
    {
        LOG_E("get device(%s) failed.", "sim1");
        return -1;
    }

    resp = at_create_resp(SIM76XX_NTP_RESP_SIZE, 4, SIM76XX_NTP_RESP_TIMO);
    if (resp == RT_NULL)
    {
        LOG_E("no memory for resp create.");
        return -1;
    }
    for (int i = 0; i < NTP_RETRY; i++)
    {
        if (at_obj_exec_cmd(device->client, resp, "AT+CNTP") < 0)
        {
            rt_thread_mdelay(500);
            continue;
        }
        if (at_resp_parse_line_args_by_kw(resp, "+CNTP:", "+CNTP: %d", &result_code) < 0)
        {
            rt_thread_mdelay(500);
            continue;
        }
        ...
        {
            LOG_D("sim ntp sync fail, retrying...");
            rt_thread_mdelay(500);
        }
    }
    if (ret != 0)
...
#define CCLK_RETRY                     10
    ...
    for (int i = 0; i < CCLK_RETRY; i++)
    {
        ...
        if (year < 20)
        {
            LOG_D("sim ntp get fail, retrying...");
            rt_thread_mdelay(500);
            continue;
        }
...
exit:
    at_delete_resp(resp);
    if(ret) LOG_W("sim ntp sync fail");
    else LOG_I("sim ntp sync success!");
    return ret;
}
